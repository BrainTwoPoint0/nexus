...
export type RemoteWorkPreference = 
  | 'no'
  | 'hybrid'
  | 'full'
  | 'occasional';
...
export function isValidAvailabilityStatus(value: string): value is AvailabilityStatus {
  return Object.keys(AVAILABILITY_STATUS_LABELS).includes(value as AvailabilityStatus);
}

export function isValidRemoteWorkPreference(value: string): value is RemoteWorkPreference {
  return Object.keys(REMOTE_WORK_LABELS).includes(value as RemoteWorkPreference);
}
...
export function parseAvailabilityFromText(text: string): AvailabilityStatus | null {
...
export function parseRemoteWorkFromText(text: string): RemoteWorkPreference | null {
  const lowercaseText = text.toLowerCase();
  ...
  // Pattern matching
  if (/fully.*remote|completely.*remote|remote.*only|work.*from.*home/i.test(text)) {
    return 'full';
  }
...
export const AVAILABILITY_STATUS_VALUES: AvailabilityStatus[] = Object.keys(AVAILABILITY_STATUS_LABELS) as AvailabilityStatus[];
export const REMOTE_WORK_VALUES: RemoteWorkPreference[] = Object.keys(REMOTE_WORK_LABELS) as RemoteWorkPreference[];
export const USER_ROLE_VALUES: UserRole[] = Object.keys(USER_ROLE_LABELS) as UserRole[];
...
export function getEnumOptionsForAI(enumType: 'availability' | 'remote_work' | 'user_role'): string {
  switch (enumType) {
    case 'availability':
