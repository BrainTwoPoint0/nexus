...
export function generateProfessionalHeadline(cvData: any): string {
  ...
  if (cvData.workHistory?.length > 0) {
    const currentJob = cvData.workHistory.find((job: any) => job.is_current) || cvData.workHistory[0];
    if (currentJob) {
  ...
  if (cvData.boardExperience?.length > 0) {
    const currentBoard = cvData.boardExperience.find((board: any) => board.is_current) || cvData.boardExperience[0];
    if (currentBoard) {
...
export function generateSmartBio(cvData: any): string {
  ...
  if (cvData.boardExperience?.length > 0) {
    ...
    const currentBoards = cvData.boardExperience.filter((b: any) => b.is_current);
...
export function generateSectorRecommendations(cvData: any): string[] {
  ...
  if (cvData.workHistory?.length > 0) {
    cvData.workHistory.forEach((job: any) => {
      ...
      if (company.includes('amazon') || company.includes('google') || company.includes('microsoft') || 
          company.includes('software') || company.includes('tech') || job.title?.toLowerCase().includes('engineer')) {
        sectors.add('Technology');
      }
      
      // Finance sector indicators
      if (company.includes('bank') || company.includes('capital') || company.includes('finance') || 
          company.includes('investment') || job.title?.toLowerCase().includes('financial')) {
        sectors.add('Finance');
      }
      
      // Healthcare sector indicators
      if (company.includes('health') || company.includes('medical') || company.includes('pharma') || 
          company.includes('hospital')) {
        sectors.add('Healthcare');
      }
      
      // Education sector indicators
      if (company.includes('university') || company.includes('college') || company.includes('school') || 
          job.title?.toLowerCase().includes('professor') || job.title?.toLowerCase().includes('teacher')) {
        sectors.add('Education');
      }
      
      // Consulting indicators
      if (company.includes('consulting') || company.includes('advisory') || 
 ...
 */
export function generateIntelligentQuestions(cvData: any, missingFields: any[]): string[] {
  ...
  missingFields.forEach(field => {
    switch(field.field) {
      ...
      case 'bio':
        ...
        questions.push(`I've drafted a professional summary based on your CV: "${generateSmartBio(cvData).substring(0, 100)}..." Would you like me to use this, or would you prefer to describe yourself differently?`);
        break;
      ...
      case 'skills':
        ...
        if (suggestedSkills.length > 0) {
          questions.push(`Based on your experience as ${headline}, I'd suggest highlighting skills like ${suggestedSkills.slice(0, 3).join(', ')}. What other key skills would you like to add?`);
        } else {
      ...
      case 'availability_status':
        questions.push(`As a ${headline}, when would you be available for new opportunities? Are you immediately available, available in 3 months, 6 months, not currently available, or would it be by arrangement?`);
        break;
        
      case 'remote_work_preference':
        questions.push("What's your preferred work arrangement - fully remote, hybrid, on-site, or flexible?");
        break;
      ...
      case 'compensation':
        ...
        if (seniority === 'senior') {
          questions.push('For senior roles like yours, what compensation range are you targeting?');
        } else {
          questions.push('What salary range would you be looking for in your next role?');
        }
        break;
...
function calculateYearsOfExperience(workHistory: any[]): number {
  ...
  const years = Math.floor((latestDate.getTime() - earliestDate.getTime()) / (365.25 * 24 * 60 * 60 * 1000));
...
function generateSkillSuggestions(cvData: any): string[] {
  ...
  if (title.includes('ceo') || title.includes('chief executive')) {
    suggestions.push('Strategic Planning', 'Leadership', 'P&L Management', 'Stakeholder Management');
  } else if (title.includes('cto') || title.includes('chief technology')) {
    suggestions.push('Technology Strategy', 'Digital Transformation', 'Team Leadership', 'Innovation');
  } else if (title.includes('cfo') || title.includes('chief financial')) {
    suggestions.push('Financial Planning', 'Risk Management', 'Budgeting', 'Financial Analysis');
  } else if (title.includes('engineer')) {
    suggestions.push('Software Development', 'Problem Solving', 'System Design', 'Technical Leadership');
  } else if (title.includes('manager')) {
    suggestions.push('Team Management', 'Project Management', 'Strategic Planning', 'Communication');
  } else if (title.includes('director')) {
    suggestions.push('Leadership', 'Strategic Planning', 'Business Development', 'Team Building');
  }
...
function estimateSeniority(cvData: any): 'junior' | 'mid' | 'senior' {
  const title = (cvData.currentRole || cvData.title || '').toLowerCase();
  const yearsExperience = cvData.workHistory ? calculateYearsOfExperience(cvData.workHistory) : 0;
  ...
  if (title.includes('chief') || title.includes('ceo') || title.includes('cto') || 
      title.includes('cfo') || title.includes('vp') || title.includes('vice president') ||
      title.includes('director') || hasBoardExperience) {
    return 'senior';
  }
